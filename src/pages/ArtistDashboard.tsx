...
type Track = { id: string; title: string; fileName: string; audioUrl?: string };
type Release = { id: string; title: string; type: "Single" | "Album"; status: "Live" | "Draft"; price: string; image: string; tracks: Track[]; redemptions: number; limit: number };

const initialReleases: Release[] = [
  { id: "synthetic", title: "Synthetic Horizons", type: "Album", status: "Live", price: "9.99", image: FALLBACK_COVER, tracks: [{ id: "t1", title: "Neon Pulse", fileName: "Preview not uploaded" }, { id: "t2", title: "Digital Rain", fileName: "Preview not uploaded" }], redemptions: 12402, limit: 15000 },
  { id: "neon", title: "Neon Pulse", type: "Single", status: "Live", price: "2.99", image: "https://lh3.googleusercontent.com/aida-public/AB6AXuCal__rmJC6rH62tAtPXBhHWj0BqHK5N77copHmQO2GPo3Z9ZsucqeKO3t6QjkMzeRA2lm6x9wrdHXN643adSUSRiHA22mxrlAqQ3IOJn-gDzQGzXbQWQ0QssD255XkJ7M6ImwW-1dnZdBHatsHn5n5No4O6f2IvJULdBp8uS6x3sDESSGijebKVKueF100dM5i09YxyNOAjFWmyXYGe4XGaJ7UdvNY1E0A_h0UPKHnOeNLWjP_lcvU9TsrAFyQ8rIT4SJfKyjFnac", tracks: [{ id: "t3", title: "Neon Pulse", fileName: "Preview not uploaded" }], redemptions: 8116, limit: 10000 },
];
...
export function ArtistDashboard() {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-xl px-container-margin pb-32 pt-24">
        ...
        <section className="relative overflow-hidden rounded-2xl border border-outline-variant/20 bg-surface-container p-xl">
          ...
          <div className="relative flex flex-col gap-lg md:flex-row md:items-end md:justify-between">
            <div className="flex items-center gap-lg">
              ...
              <div><span className="font-label-md text-label-md uppercase tracking-widest text-primary">Artist workspace</span><h1 className="mt-xs font-display-lg text-display-lg leading-none">Cyber Echoes</h1><p className="mt-sm font-body-md text-body-md text-secondary">Verified artist · Electronic · New York</p></div>
            </div>
      ...
      {studioOpen && <UploadStudio onClose={() => setStudioOpen(false)} onSave={(release) => { setReleases((items) => [release, ...items]); setStudioOpen(false); }} />}
    </main>
  );
}
...
function UploadStudio({ onClose, onSave }: { onClose: () => void; onSave: (release: Release) => void }) {
  ...
  const updateTrack = (id: string, patch: Partial<Track>) => setTracks((items) => items.map((track) => track.id === id ? { ...track, ...patch } : track));
  const submit = (event: FormEvent, status: "Live" | "Draft") => { event.preventDefault(); if (!title.trim() || tracks.some((track) => !track.title.trim() || !track.audioUrl)) return; onSave({ id: crypto.randomUUID(), title, type, status, price, image: cover, tracks, redemptions: 0, limit: 10000 }); };

  return <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/80 p-0 backdrop-blur-sm md:items-center md:p-lg" role="dialog" aria-modal="true" aria-label="Upload music"><div className="max-h-[94dvh] w-full max-w-4xl overflow-y-auto rounded-t-2xl border border-outline-variant/20 bg-background md:rounded-2xl"><div className="sticky top-0 z-20 flex items-center justify-between border-b border-outline-variant/15 bg-background/90 p-lg backdrop-blur-xl"><div><span className="font-label-md text-label-md uppercase tracking-widest text-primary">Artist upload studio</span><h2 className="font-headline-lg text-headline-lg">Create a release</h2></div><button onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-full bg-surface-container-high text-secondary hover:text-white"><Icon name="close"/></button></div><form className="grid grid-cols-1 gap-lg p-lg md:grid-cols-3"><div className="space-y-md"><label className="group relative block aspect-square cursor-pointer overflow-hidden rounded-xl border border-dashed border-outline bg-surface-container-low"><img src={cover} alt="Cover preview" className="h-full w-full object-cover opacity-70"/><div className="absolute inset-0 flex flex-col items-center justify-center bg-black/30"><Icon name="add_photo_alternate" className="text-[42px] text-primary"/><span className="mt-xs font-label-md text-label-md">UPLOAD COVER</span></div><input type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) setCover(URL.createObjectURL(file)); }}/></label><p className="text-center font-label-sm text-label-sm text-secondary">Square JPG, PNG or WEBP</p></div><div className="space-y-lg md:col-span-2"><div className="grid grid-cols-2 gap-xs rounded-xl bg-surface-container-low p-1">{(["Single","Album"] as const).map((item) => <button key={item} type="button" onClick={() => changeType(item)} className={`rounded-lg py-sm font-label-md text-label-md ${type === item ? "bg-surface-container-highest text-primary" : "text-secondary"}`}>{item.toUpperCase()}</button>)}</div><div className="grid grid-cols-1 gap-md sm:grid-cols-[1fr_140px]"><Field label="Release title"><input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder={type === "Single" ? "Single title" : "Album title"} className="artist-input"/></Field><Field label="Price (USD)"><input required type="number" min="0" step="0.01" value={price} onChange={(e) => setPrice(e.target.value)} className="artist-input"/></Field></div><div><div className="mb-sm flex items-center justify-between"><div><h3 className="font-body-lg text-body-lg font-bold">Tracks & samples</h3><p className="font-label-sm text-label-sm text-secondary">Upload the full audio file. The store exposes only the first 30 seconds.</p></div>{type === "Album" && <Button type="button" size="sm" variant="outline" onClick={addTrack}><Icon name="add"/>Track</Button>}</div><div className="space-y-sm">{tracks.map((track, index) => <div key={track.id} className="rounded-xl border border-outline-variant/15 bg-surface-container-low p-md"><div className="flex items-center gap-sm"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-surface-container-high text-primary">{index + 1}</span><input required value={track.title} onChange={(e) => updateTrack(track.id, { title: e.target.value })} placeholder="Track title" className="artist-input min-w-0 flex-1"/>{type === "Album" && tracks.length > 1 && <button type="button" onClick={() => setTracks((items) => items.filter((item) => item.id !== track.id))} className="text-secondary hover:text-error"><Icon name="delete"/></button>}</div><label className="mt-sm flex cursor-pointer items-center justify-between rounded-lg bg-surface-container-high px-md py-sm"><div className="flex min-w-0 items-center gap-sm"><Icon name={track.audioUrl ? "check_circle" : "audio_file"} className={track.audioUrl ? "text-primary" : "text-secondary"}/><span className="truncate text-body-md">{track.fileName || "Choose WAV, MP3, FLAC or M4A"}</span></div><span className="font-label-sm text-label-sm text-primary">BROWSE</span><input required type="file" accept="audio/*,.wav,.mp3,.flac,.m4a" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) updateTrack(track.id, { fileName: file.name, audioUrl: URL.createObjectURL(file) }); }}/></label>{track.audioUrl && <audio controls src={track.audioUrl} className="mt-sm h-9 w-full"/>}</div>)}</div></div><div className="flex flex-col-reverse gap-sm border-t border-outline-variant/15 pt-lg sm:flex-row sm:justify-end"><Button type="button" variant="ghost" onClick={onClose}>Cancel</Button><Button type="button" variant="outline" onClick={(e) => submit(e, "Draft")}><Icon name="save"/>Save draft</Button><Button type="submit" onClick={(e) => submit(e, "Live")}><Icon name="publish"/>Publish release</Button></div></div></form></div></div>;
}
